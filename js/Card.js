export default class Card {
  ...
  _handleOpenPopup(imagePopup) {
    ...
    this._pageSelector.addEventListener('keydown', this._handleCloseEvent);
    this._pageSelector.addEventListener('click', this._handleCloseEvent);
  }

  _handleClosePopup(popup) {
    console.log(popup);
    popup.classList.remove('popup_opened');
    this._pageSelector.removeEventListener('keydown', this._handleCloseEvent);
    this._pageSelector.removeEventListener('click', this._handleCloseEvent);
  }
  ...
  _handleCloseEvent(evt) {
    ...
    if (evt.target.classList.contains('popup') || evt.key == 'Escape') {
      ...
      console.log(evt.key);
      console.log(this._handleClosePopup);
